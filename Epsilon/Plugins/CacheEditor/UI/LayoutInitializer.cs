...
namespace CacheEditor
{
    public class LayoutInitializer : ILayoutUpdateStrategy
    {
        ...
        {
            var tool = anchorableToShow.Content as ICacheEditorTool;
            if (tool != null)
            {
        ...
        public void AfterInsertAnchorable(LayoutRoot layout, LayoutAnchorable anchorableShown)
        {
            ...
            var tool = anchorableShown.Content as ICacheEditorTool;
            if (tool != null)
            {
                var anchorablePane = anchorableShown.Parent as LayoutAnchorablePane;
                if (anchorablePane != null)
                {
                    if(tool.InitialAutoHidden)
                    ...
                    switch (tool.PreferredLocation)
                    {
                        ...
                        case PaneLocation.Right:
                            ...
                            anchorableShown.AutoHideWidth = tool.PreferredWidth;
                            anchorableShown.FloatingWidth = tool.PreferredWidth;
                            anchorableShown.FloatingHeight = 600;
                            break;
                        case PaneLocation.Bottom:
                            anchorablePane.DockHeight = new GridLength(tool.PreferredHeight, GridUnitType.Pixel);
                            anchorableShown.AutoHideHeight = tool.PreferredHeight;
